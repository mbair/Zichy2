        ...
        <div class="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
            ...
            <div class="inline-flex items-center">
                <p-iconfield>
                    ...
                    <input pInputText type="text" (input)="onFilterGlobal($event)" placeholder="Search" [style]="{ borderRadius: '2rem' }" class="w-full" />
                </p-iconfield>
                <p-button icon="pi pi-upload" class="mx-4 export-target-button" rounded pTooltip="Export" (click)="dt.exportCSV()" />
            </div>
        </div>
        <p-table #dt [value]="products" [paginator]="true" [rows]="5" [columns]="cols" responsiveLayout="scroll" [globalFilterFields]="['name', 'category', 'price', 'inventoryStatus']" [exportHeader]="'customExportHeader'">
            <ng-template #header>
                <tr>
            ...
            <ng-template #body let-product>
                <tr>
                    ...
                    <td style="width: 35%; min-width: 8rem;">
                        <p-tag [severity]="getBadgeSeverity(product.inventoryStatus)">{{ product.inventoryStatus }}</p-tag>
                    </td>
                    <td style="width: 15%;">
