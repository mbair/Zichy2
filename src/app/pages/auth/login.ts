...
@Component({
    selector: 'app-login',
    standalone: true,
    imports: [CheckboxModule, InputTextModule, FormsModule, RouterModule, AppConfigurator, IconFieldModule, InputIconModule, ButtonModule],
    template: `
            ...
            <div class="border border-surface-200 dark:border-surface-700 bg-surface-0 dark:bg-surface-900 rounded py-16 px-6 md:px-16 z-10">
                ...
                <div class="flex flex-col">
                    <p-iconfield class="w-full mb-6">
                        ...
                        <input id="email" type="text" pInputText class="w-full md:w-100" placeholder="Email" />
                    </p-iconfield>

                    <p-iconfield class="w-full mb-6">
                        <p-inputicon class="pi pi-lock" />
                        <input id="password" type="password" pInputText class="w-full md:w-100" placeholder="Password" />
                    </p-iconfield>

                    <div class="mb-6 flex flex-wrap gap-4">
                        <div class="flex items-center">
                            <p-checkbox name="checkbox" value="val" [(ngModel)]="rememberMe" styleClass="mr-2" class="flex" [binary]="true"></p-checkbox>
                            <label for="checkbox" class="text-surface-900 dark:text-surface-0 font-medium mr-20">Remember Me</label>
                        </div>
                        <a [routerLink]="[apolloRoutes.auth.forgotPassword]" class="text-surface-600 dark:text-surface-200 cursor-pointer hover:text-primary ml-auto transition-colors duration-300">Reset password</a>
                    </div>
                    <button pButton pRipple label="Log In" class="w-full" [routerLink]="[apolloRoutes.root]"></button>
                </div>
            </div>
        </div>
