...
export class MailDetail implements OnInit {
    ...
    async ngOnInit() {
        ...
        if (Number.isNaN(parsedId)) {
            this.router.navigate([APOLLO_ROUTES.notFound]);
            return;
        }

        this.emailId.set(parsedId);
        this.fromView.set(this.route.snapshot.queryParams['from'] || 'Inbox');

        if (!this.mailService.getEmailById(parsedId)) {
            this.router.navigate([APOLLO_ROUTES.notFound]);
        }
    }
    ...
    goBack() {
        this.router.navigate([APOLLO_ROUTES.apps.mailInbox], { queryParams: { view: this.fromView() } });
    }
