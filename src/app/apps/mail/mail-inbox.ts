...
const DEFAULT_CATEGORY_ITEMS: MenuItemData[] = [{ label: 'Updates', icon: 'pi pi-info-circle' }, { label: 'Social', icon: 'pi pi-users' }, { label: 'Forms', icon: 'pi pi-file' }, { label: 'Introductions', icon: 'pi pi-comments' }];
...
@Component({
    ...
    template: `
        <div class="flex h-full relative card">
            ...
            <!-- Mobile Drawer -->
            <p-drawer [(visible)]="showMenuDrawer" header="Mails" position="left" styleClass="w-[280px]">
...
export class MailInbox implements OnInit {
    ...
    async ngOnInit() {
        ...
        try {
            ...
            this.menuItems.set(data.menuItems ?? DEFAULT_MENU_ITEMS);
            this.categoryItems.set(data.categoryItems ?? DEFAULT_CATEGORY_ITEMS);
        } catch (error) {
            console.error('Failed to load mail menu metadata:', error);
            this.menuItems.set(DEFAULT_MENU_ITEMS);
            this.categoryItems.set(DEFAULT_CATEGORY_ITEMS);
        }
    ...
    navigateToEmail(email: Email) {
        ...
        this.router.navigate([APOLLO_ROUTES.apps.mailDetail, email.id], { queryParams: { from } });
    }
