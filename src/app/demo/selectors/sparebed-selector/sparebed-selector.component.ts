...
export class SparebedSelectorComponent implements OnInit, AfterViewChecked, ControlValueAccessor {
    ...
    private syncValueFromWidget(): void {
        ...
        if (widgetValue === undefined || widgetValue === null || widgetValue === this.selectedSparebed) {
            return
        }
    ...
    private applySelection(value: string, emitTouch = true, emitCva = true): void {
        ...
        if (this.selectedSparebed !== normalized) {
            this.selectedSparebed = normalized
        }
