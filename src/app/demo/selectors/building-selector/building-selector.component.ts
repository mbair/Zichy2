...
export class BuildingSelectorComponent implements OnInit, AfterViewChecked, ControlValueAccessor {
    ...
    private syncValueFromWidget(): void {
        ...
        if (widgetValue === undefined || widgetValue === null || widgetValue === this.selectedBuilding) {
            return
        }
    ...
    private applySelection(value: string, emitTouch = true, emitCva = true): void {
        ...
        if (this.selectedBuilding !== normalized) {
            this.selectedBuilding = normalized
        }
